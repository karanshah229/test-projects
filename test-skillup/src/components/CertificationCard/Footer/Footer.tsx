...
export type CertificationCardFooterType = {
  assigned_by: AssignmentAssignedByType;
  due_date: string | null;
};

export function CertificationCardFooter(props: CertificationCardFooterType) {
  const { assigned_by: assignedBy = {} as AssignmentAssignedByType, due_date: dueDate = null } =
    props;

  const { first_name: firstName = '', last_name: lastName = '' } = assignedBy;

  return (
    <div className={`hr-flex hr-col bg-white hr-p-1 ${styles.card_footer}`}>
        ...
        {dueDate ? (
          <div
            className={`hr-flex hr-align-center hr-gap-0.25 ${
              dayjs(dueDate).isBefore(new Date()) ? styles.card_footer__overdue : ''
            }`}
          >
